...
@forelse ( $request_call as $key => $item )
...
@media (max-width: 767px) {
    ...
    .cu_contact_item {
        font-size: 14px;
        line-height: 1; /* Adjust this value to reduce/increase line spacing */
        margin-bottom: 5px; /* Add margin between items for better spacing */
    }
    
    /* Change the flex direction to display items vertically */
    .cu_contact {
...
<h1><i class="fa fa-phone" ></i> {{$item->number}} </h1>

<h1><i class="fa fa-map-marker" ></i> {{$item->email}}</h1>
...
<a class = "reject" href = "{{ route('request_action_rejected', $item->tendorId) }}">Reject</a>
  ...
  @empty
  <p class="req_call_empty mt-4">No Request Call Found...!</p>
...
@forelse ( $request_call_action as $key => $item )
